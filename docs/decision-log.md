...
it below `aside#usercentrics-cmp-ui`, match only the observed `Accept all`, `Accept all
cookies`, and `Continue in English` actions, click normally, retain Playwright's default
post-handler disappearance wait, and remove the handler after two invocations. Prove
delayed handling, bounded exhaustion, unrelated action failure, and arbitrary-dialog
isolation against in-memory page content.
